...
class FileUploadService {
    constructor() {
      ...
      this.frameworkToVaultCategoryMap = {
        ...
        'ISO 50001': 'energy-docs',    // Added ISO 50001 Energy Management
        'ISO 14064': 'environmental',  // Added ISO 14064 GHG Emissions
    ...
    determineCategory(file, options = {}) {
      ...
      if (workflow && workflow.title) {
        ...
        if (workflow.title.includes('ISO')) {
          ...
          if (workflow.title.includes('50001')) {
            console.log(`Categorizing "${file.name}" as "${this.frameworkToVaultCategoryMap['ISO 50001']}" based on ISO 50001 in workflow title`);
            return this.frameworkToVaultCategoryMap['ISO 50001'];
          }
          if (workflow.title.includes('14064')) {
            console.log(`Categorizing "${file.name}" as "${this.frameworkToVaultCategoryMap['ISO 14064']}" based on ISO 14064 in workflow title`);
            return this.frameworkToVaultCategoryMap['ISO 14064'];
          }
    ...
    async processQueue() {
      try {
        ...
        while (this.uploadQueue.length > 0) {
          ...
          const { file, fileId, category, options } = this.uploadQueue.shift();
    ...
    getCategoryLabel(category) {
      // Simple formatting - actual translations handled by Trans components in UI
