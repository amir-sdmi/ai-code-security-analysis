...
// Simple file logger for Gemini requests
function logGeminiRequest(details: unknown): void {
    try {
        const logsDir = path.join(process.cwd(), 'logs');
        fs.mkdirSync(logsDir, { recursive: true });
        const logPath = path.join(logsDir, 'gemini_requests.log');
        const entry = `[${new Date().toISOString()}] ${JSON.stringify(details)}\n`;
        fs.appendFileSync(logPath, entry);
    } catch (error) {
        console.error('Failed to write Gemini request log:', error);
    }
}
...
// Normalize speaker names for consistent lookups
...
async function generateMultiSpeakerPodcast(
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        const hostCfg = speakerMap[normalizeSpeakerName(spkA)] || { voice: DEFAULT_HOST_VOICE, tone: '' };
        const guestCfg = spkB ? speakerMap[normalizeSpeakerName(spkB)] || { voice: DEFAULT_GUEST_VOICE, tone: '' } : hostCfg;
...
// Helper to create a fresh Google Gemini client for each request
...
async function retryGeminiTTS(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            logGeminiRequest({ batchIndex: batchIndex + 1, attempt, requestConfig });
...
export async function POST(req: NextRequest) {
    try {
        ...
        // Create a fresh Gemini client for this request
        ...
        if (numSpeakers > 2) {
            ...
            for (let i = 0; i < finalSpeakerNames.length; i++) {
                ...
                speakerMap[normalizeSpeakerName(name)] = { voice: voice || DEFAULT_HOST_VOICE, tone: tone || '' };
            }
