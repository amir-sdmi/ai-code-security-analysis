...
// Supabase Edge Function URL - replace with your actual URL after deployment
const SUPABASE_EDGE_FUNCTION_URL = "https://ruynjrxuleadlblizuuj.supabase.co/functions/v1/gemini-proxy";
...
export class GeminiService {
  ...
  async generateImageWithImagen3(
    ...
    try {
      ...
      const response = await fetch(SUPABASE_EDGE_FUNCTION_URL, {
        ...
        body: JSON.stringify({
          endpoint: `${IMAGEN3_MODEL}:generateImages`,
          apiKey: this.apiKey,
          payload: payload
        })
      });
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.error?.code === 400) {
          ...
          toast.error(`Error: ${errorData.error?.message || "Unknown error"}`);
        }
        return null;
      }
  ...
  private async editImage(instructions: string, imageUrl: string): Promise<ChatMessage | null> {
    try {
      ...
      const data = await this.makeApiRequest(
        `${IMAGE_EDITING_MODEL}:generateContent`,
        payload
      );
