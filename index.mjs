...
// Rate‐limit guard for core 5k‐per‐hour bucket
async function guardCore() {
  ...
  if (rate.remaining < 50) {
    ...
    console.log(`🛑 CORE limit low; sleeping ${Math.ceil(ms / 1000)}s`);
    ...
    for (; state.i < AI_TOOLS.length; state.i++, state.j = 0, state.k = 0) {
      ...
      for (; state.j < SEARCH_TERMS.length; state.j++, state.k = 0) {
        ...
        for (; state.k < LANGUAGES.length; state.k++) {
          ...
          console.log(`🔍 [${state.i},${state.j},${state.k}] ${query}`);
          ...
          while (state.page <= 30) {
            ...
            try {
              ...
              // Secondary‐limit (abuse) wrapper
              if (
                err.status === 403 &&
                /secondary rate limit/i.test(err.message)
              ) {
                console.warn("🚦 secondary limit – sleeping 60s");
            ...
            for (const it of items) {
              ...
              try {
                ...
                console.warn(`⚠️ skipping ${it.html_url}: ${e.message}`);
              }
            }
